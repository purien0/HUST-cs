...
inline bool operator==(struct node a,struct node b)
{
...
int main()
{
	...
	{
		num = (x[n/2]-x[n/2-1]+1)*(y[n/2]-y[n/2-1]+1);
		for(int k = 0; k < n; k++)
		{
			sum +=abs(arr[k].x-x[n/2])+abs(arr[k].y-y[n/2]);//找(i,j) 
			if(arr[k].x<=x[n/2]&&arr[k].x>=x[n/2-1]&&arr[k].y<=y[n/2]&&arr[k].y>=y[n/2-1])
			num--;
		}
		ans = sum;
	}
